...
class ProjectController extends Controller
{
    ...
    public function projUpdate($id, Request $request, Project $proj)
    {
        ...
        if (!$proj) {
            ...
            if ($request->contractor_id == null) {
                ...
                $con = faq_project_contractor::where('project_id', $id)->first();
                if ($con) {
                    faq_project_contractor::where('project_id', $id)->delete();
                }
            }
            if ($request->contractor_id != null) {
    ...
    public function projectDetailsCont($id)
    {
        $proj = Project::where('project_id', $id)->get();
        if ($proj->isEmpty()) {
    ...
    public function project_details()
    {
        ...
        if ($proj->isEmpty()) {
            ...
            return response()->json([
                ...
                "canceled_project" => sizeof($quoted)
            ], 200);
            // $data=$proj;
            // $code=200;
        }
    ...
    public function projectData()
    {
        ...
        if ($exp->isEmpty()) {
            ...
            // foreach($exp_acc as $e){
            //     $e->setAttribute('project_id',$e->id);
            //     $e->setAttribute('project_name',$e->expense_acc_name);
            // }
            // $exp=$exp->merge($exp_acc);
            // return gettype($exp_acc);
            // $exp=$exp->concat($exp_acc);
    ...
    public function comp_project($id)
    {
        ...
        $amount = 0;
        // $a=[];
        $c = company::where('company_name', $id)->first();
        $proj = Project::where("company_id", $c->company_id)->get();
